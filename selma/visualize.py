...
OUTPUT_FILE = os.path.join(BASE_DIR, "visualize.html")
...
<head>
    ...
    <title>SELMA - Lottery Analysis</title>
...
<body>
    <div class="sidebar">
        ...
        <h2>Analysis</h2>
        <div class="section">
            <div class="section-item active" data-view="profiles">Profiles</div>
            <div class="section-item" data-view="oddeven">Odd / Even</div>
            <div class="section-item" data-view="numgroups">Number Groups</div>
            <div class="section-item" data-view="sumrange">Sum Range</div>
            <div class="section-item" data-view="distance">Recency</div>
            <div class="section-item" data-view="consecutive">Consecutive</div>
            <div class="section-item" data-view="hotcold">Hot / Cold</div>
        </div>
    </div>

    <div class="main">
        <!-- Profiles view -->
        <div id="view-profiles" class="view active">
        ...
        <!-- Odd/Even view -->
        <div id="view-oddeven" class="view">
        ...
        <!-- Number Groups view -->
        <div id="view-numgroups" class="view">
            <h2>Number Groups</h2>
        ...
        <!-- Sum Range view -->
        <div id="view-sumrange" class="view">
            <h2>Sum Range</h2>
            <p style="font-size: 13px; color: #888; margin-bottom: 15px;">
        ...
        <!-- Recency / Distance view -->
        <div id="view-distance" class="view">
            <h2>Recency Score</h2>
            <p style="font-size: 13px; color: #888; margin-bottom: 15px;">
        ...
        <!-- Consecutive view -->
        <div id="view-consecutive" class="view">
        ...
        <!-- Hot/Cold view -->
        <div id="view-hotcold" class="view">
    ...
    const yearDrawCounts = YEAR_DRAW_COUNTS_PLACEHOLDER;
    const oddevenFreq = ODDEVEN_FREQ_PLACEHOLDER;
    const oddevenOcc = ODDEVEN_OCC_PLACEHOLDER;
    const templatesOcc = TEMPLATES_OCC_PLACEHOLDER;
    const sumrangeOcc = SUMRANGE_OCC_PLACEHOLDER;
    const distanceFreq = DISTANCE_FREQ_PLACEHOLDER;
    const distanceScores = DISTANCE_SCORES_PLACEHOLDER;
    const consecutiveOcc = CONSECUTIVE_OCC_PLACEHOLDER;
    const hotcoldFreq = HOTCOLD_FREQ_PLACEHOLDER;
    ...
    document.querySelectorAll('.section-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (item.dataset.view === 'oddeven') initOddEven();
            if (item.dataset.view === 'numgroups') initNumGroups();
            if (item.dataset.view === 'sumrange') initSumRange();
            if (item.dataset.view === 'distance') initDistance();
            if (item.dataset.view === 'consecutive') initConsecutive();
            if (item.dataset.view === 'hotcold') initHotCold();
        });
    });
    ...
    let oddevenInitialized = false;

    function initOddEven() {
        if (oddevenInitialized) {
            return;
        }
        oddevenInitialized = true;
    ...
    let numgroupsInitialized = false;

    const decadeLabels = ['1-9', '10-19', '20-29', '30-39', '40-49'];
    const decadeColors = ['#e6194b', '#3cb44b', '#4363d8', '#f58231', '#911eb4'];

    function initNumGroups() {
        if (numgroupsInitialized) return;
        numgroupsInitialized = true;
    ...
    function updateNumGroupsCharts() {
        ...
        // spread distribution: how many unique decade groups are covered per draw
        // e.g. draw [3,12,25,31,40,47] = groups [0,1,2,3,4,4] = 5 of 5 decades covered
        const spreadCounts = {};
        filtered.forEach(d => {
            const s = d.spread;
    ...
    let sumrangeInitialized = false;

    function initSumRange() {
        if (sumrangeInitialized) return;
        sumrangeInitialized = true;
    ...
    let consecInitialized = false;

    function initConsecutive() {
        if (consecInitialized) return;
        consecInitialized = true;
    ...
    let hotcoldInitialized = false;

    function initHotCold() {
        if (hotcoldInitialized) return;
        hotcoldInitialized = true;
    ...
    # Embed data
    html = html.replace('PROFILE_DATA_PLACEHOLDER', json.dumps(profile_data))
    html = html.replace('YEARS_PLACEHOLDER', json.dumps(years))
    html = html.replace('YEAR_DRAW_COUNTS_PLACEHOLDER', json.dumps(year_draw_counts))
    html = html.replace('ODDEVEN_FREQ_PLACEHOLDER', json.dumps(oddeven_freq))
    html = html.replace('ODDEVEN_OCC_PLACEHOLDER', json.dumps(oddeven_occ))
    html = html.replace('TEMPLATES_OCC_PLACEHOLDER', json.dumps(templates_occ))
    html = html.replace('SUMRANGE_OCC_PLACEHOLDER', json.dumps(sumrange_occ))
    html = html.replace('DISTANCE_FREQ_PLACEHOLDER', json.dumps(distance_freq))
    html = html.replace('DISTANCE_SCORES_PLACEHOLDER', json.dumps(distance_scores))
    html = html.replace('CONSECUTIVE_OCC_PLACEHOLDER', json.dumps(consecutive_occ))
    html = html.replace('HOTCOLD_FREQ_PLACEHOLDER', json.dumps(hotcold_freq))

    with open(OUTPUT_FILE, 'w') as f:
        f.write(html)

    print("Written to " + OUTPUT_FILE)
    print("Open in browser: file://" + OUTPUT_FILE)
